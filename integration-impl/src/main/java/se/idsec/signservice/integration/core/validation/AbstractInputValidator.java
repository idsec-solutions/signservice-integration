...
public abstract class AbstractInputValidator<T, H> implements InputValidator<T, H> {
  ...
  /** {@inheritDoc} */
  @Override
  public void validateObject(final T object, final String objectName, final H hint, final String correlationID)
      throws InputValidationException {
    if (objectName == null) {
      throw new InputValidationException("unknown", "Bad call to validateObject");
    }
    ValidationResult errors = this.validate(object, objectName, hint, correlationID);
    if (errors.hasErrors()) {
